...
class KDTree
{
  ...
  private:
    ...
    KDTreeNode *root; // don't forget to initialize this in ctor!
    size_t size;
    //vector<Point<Dim>>* sortedPoints; // Note: my member, used to hold nodes of tree, don't forget to destruct this
    ...
    int Partition(vector<Point<Dim>>& list, int start, int end, int medianIndex, int dim);
    void constructorHelper(vector<Point<Dim>>& list, KDTreeNode*& subroot, int left, int right, int curDim); // recursive, sorts and builds tree
    void copyConstructorHelper(KDTreeNode*& copy, KDTreeNode* original);
    void destructorHelper(KDTreeNode*& subroot);
    void nearestNeighborHelper(const Point<Dim>& target, KDTreeNode* subroot, Point<Dim>& closest, int curDim, double& radius) const;
};
